        ...
        foreach (var value in ids)
        {
            ...
            if (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
            {
                ...
                var tempo = reader.IsDBNull(4) || reader.IsDBNull(5)
                    ? null
                    : new Tempo(reader.GetDouble(4), reader.GetDouble(5));
    ...
    public async Task UpsertAsync(AudioAnalysisRow row, CancellationToken cancellationToken)
    {
        ...
        command.CommandText = """
            INSERT INTO audio_analysis
                (jellyfin_id, mtime_ticks, loudness_lufs, true_peak_dbfs, bpm, bpm_confidence, source, error, measured_at)
            VALUES ($id, $mtime, $lufs, $peak, $bpm, $confidence, $source, $error, $at)
            ON CONFLICT (jellyfin_id) DO UPDATE SET
                mtime_ticks = $mtime, loudness_lufs = $lufs, true_peak_dbfs = $peak,
                bpm = $bpm, bpm_confidence = $confidence,
